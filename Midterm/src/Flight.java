...
public class Flight
{
    ...
    /**
        ...
        * is not in the Flight passenger list already. <-for each and if loop to check
         * When deciding whether a name is a passenger already or
         * not, the  name  is matched against the passenger list names
         * regardless of (upper/lower) case and regardless of leading and trailing blanks. <-trim and ignorecase
         *  
         * If the  name is added to the  Flight , and if  isAdult  is true ,
         * the number of adults goes up by one. <-adults++
    ...
    {
        //check if name already exists, if so return false
        for (String x : names) {
            if (x.equalsIgnoreCase(name.trim())) {
                return false;
            }
        }
    ...
    {
        return flightNum + " {passengers:" + names.size() + " adults:" + adults + " " + names + "}";
    }
    ...
    /**
     ...
     public int totalNoDups(Flight other)  
     {
         ...
         int nodups;
         nodups = names.size() + other.names.size();

         for (String x : names) {
             for (String y : other.names) {
